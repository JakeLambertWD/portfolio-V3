...
export default function Skill({ directionBottom, tech }: Props) {
  ...
  return (
    <div className="relative flex cursor-pointer group tooltip" data-tip={tech?.title}>
      <motion.img initial={{ y: directionBottom ? -200 : 200, opacity: 0 }} transition={{ duration: 1 }} whileInView={{ y: 0, opacity: 1 }} src={urlFor(tech?.image).url()} className={clsx("object-contain w-24 h-24 filter transition duration-100 ease-in rounded-lg", "md:h-28 md:w-28", "xl:w-32 xl:h-32", "group-hover:grayscale group-hover:blur-sm")} />
